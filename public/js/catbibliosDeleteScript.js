...
document.querySelector("#botonBorrarCatbiblios").addEventListener("click", function () {
    let objeto = {
    ...
    fetch("/catbiblios/delete", fetchData) //le metemos la ruta en el servidor, y los datos que queremos que le meta. //cuando reciba la respuesta del app.get del index.js, entonces hacemos las 2 siguientes lineas.
        ...
        .then(
            data => {
                if (data.status) { //si es true, correcto.
                    alert(data.mensaje);
                    location.href = "http://localhost:3000/catbibliosGet.html";
                }
                else {
                    alert(data.mensaje);
                }
